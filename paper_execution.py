...
class PaperExecutionLoop:
    def __init__(self, account: PaperAccount, registry: Optional[CandidateRegistry] = None,
        ...
        self.account = account; self.registry = registry or CandidateRegistry(); self.monitor = monitor or PaperSessionMonitor(self.registry); self.observer = observer or PaperSessionObserver(); self.stats = ExecutionStats(); self.last_monitor_decision = None
    ...
    def on_market(self, market: dict, candidate: Optional[dict] = None, exit_signal: bool = False) -> dict:
        price = float(market["price"]); timestamp = market.get("timestamp")
        if self.account.position is not None:
            ...
            if stop_hit or target_hit or exit_signal:
                reason = "SL" if stop_hit else "TP" if target_hit else "SIGNAL"; pnl = self.account.close_position(price, reason, timestamp); self._record_close(pnl); self._record_equity(price)
                result = {"action": "CLOSE", "reason": reason, "pnl": pnl}; self._heartbeat(price, timestamp); return result
            self._record_equity(price); result = {"action": "HOLD", "equity": self.account.equity(price)}; self._heartbeat(price, timestamp); return result
        if self.registry.active() is None:
            decision = self._monitor_before_entry(price); self._record_equity(price)
            result = {"action": "WAIT", "reason": decision.reason, "monitor_status": decision.status, "monitor_reason": decision.reason}; self._heartbeat(price, timestamp); return result
        decision = self._monitor_before_entry(price)
        if str(decision.status).upper() == "ROLLBACK":
            self._record_equity(price); result = {"action": "WAIT", "reason": "paper_monitor_rollback_recovery", "monitor_status": "BLOCKED", "monitor_reason": decision.reason}; self._heartbeat(price, timestamp); return result
        if not decision.allow_new_entries:
            self._record_equity(price); result = {"action": "WAIT", "reason": "paper_monitor_blocked", "monitor_status": decision.status, "monitor_reason": decision.reason}; self._heartbeat(price, timestamp); return result
        gate = get_active_candidate(self.registry, str(market["symbol"]))
        if not gate.allowed:
            self._record_equity(price); result = {"action": "WAIT", "reason": gate.reason}; self._heartbeat(price, timestamp); return result
        active = dict(gate.active.candidate); direction = str(active.get("Direction", market.get("direction", "LONG"))).upper(); requested_direction = str(market.get("direction", direction)).upper()
        if direction != requested_direction:
            self._record_equity(price); result = {"action": "WAIT", "reason": "candidate_direction_mismatch"}; self._heartbeat(price, timestamp); return result
        try: rr = float(active.get("RR", active.get("rr", market.get("rr", 2.0))))
        ...
        self._record_equity(price); result = {"action": "OPEN", "position": position, "candidate_id": gate.active.candidate_id, "monitor_status": decision.status}; self._heartbeat(price, timestamp); return result
    ...
    def _record_equity(self, price: float) -> None: self.stats.equity_curve.append(self.account.equity(price))
