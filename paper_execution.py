...
class PaperExecutionLoop:
    ...
    def on_market(self, market: dict, candidate: Optional[dict] = None, exit_signal: bool = False) -> dict:
        ...
        # Always refresh this symbol's mark, even when other positions are open.
        self.account.equity(price, symbol)

        # Manage only the position belonging to the current market symbol. Other
        # open positions remain active and are evaluated when their own quotes arrive.
        ...
        if position is not None:
            ...
            if stop_hit or target_hit or exit_signal:
                ...
                pnl = self.account.close_position(price, reason, timestamp, symbol=symbol)
                self._record_close(pnl)
                self._record_equity(price, symbol)
                result = {"action": "CLOSE", "symbol": symbol, "reason": reason, "pnl": pnl}
                self._heartbeat(price, timestamp)
                return result
            self._record_equity(price, symbol)
            result = {"action": "HOLD", "symbol": symbol, "equity": self.account.equity()}
            self._heartbeat(price, timestamp)
            return result
        ...
        position = self.account.open_position(symbol=symbol, direction=direction, price=price,
                                               stop_distance=float(market["stop_distance"]), rr=rr,
                                               timestamp=timestamp)
        self._record_equity(price, symbol)
        result = {"action": "OPEN", "position": position, "candidate_id": candidate_id,
                  "monitor_status": getattr(locals().get("decision", None), "status", "HEALTHY"),
                  "open_position_count": len(self.account.positions)}
        self._heartbeat(price, timestamp)
        return result
    ...
    def _record_equity(self, price: float, symbol: str) -> None:
        self.stats.equity_curve.append(self.account.equity(price, symbol))
    ...
    def summary(self, *, _observe: bool = True, mark_price: Optional[float] = None) -> dict:
        ...
        result = {
            ...
            "open_symbols": sorted(self.account.positions.keys()),
