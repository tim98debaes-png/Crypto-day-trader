...
def entry_signal_details(prices: list[float], direction: str = "LONG"):
    ...
    if direction == "LONG":
        ...
        actual_pullback = any(pullback[i] < pullback[i - 1] for i in range(1, len(pullback)))
        reclaimed = price > fast * 1.001
        followthrough = trigger[-1] > trigger[0] * 1.0006 and trigger[-1] > trigger[-2]
        structure = trigger[-1] > trigger[0] * 1.0001 and trigger[-1] > min(pullback) * 1.001
        ...
        if not regime:
            return False, "trend_not_confirmed", 0, {"trend": False, "bounce_score": bounce_score, "bounce_checks": bounce_checks}
        if price > fast * 1.0065:
            return False, "overextended", 0, {"trend": True, "bounce_score": bounce_score, "bounce_checks": bounce_checks}
        if negative == 3:
            return False, "short_term_reversal", 0, {"trend": True, "bounce_score": bounce_score, "bounce_checks": bounce_checks}
    else:
        regime = fast < medium < slow and long_return < -0.0015
        ...
        structure = trigger[-1] < trigger[0] * 0.9999 and trigger[-1] < max(pullback) * 0.999
        medium_ok = medium_return < -0.0010
        short_ok = short_return < -0.0003
        micro_ok = negative >= 2
        bounce_checks = {"pullback_touch": touched, "ema_reclaim": reclaimed, "directional_followthrough": followthrough, "pullback_structure": structure}
