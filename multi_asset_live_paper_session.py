...
def run_multi_asset_paper_session(*, feed, loop: PaperExecutionLoop, duration_seconds: int = 3600,
    ...
    while clock()-started < duration_seconds:
        ...
        btc_prices=list(history.get("BTCUSDT",()))
        btc_ema=_ema(btc_prices,20) if len(btc_prices)>=5 else None
        btc_trend_ok=btc_ema is None or btc_prices[-1] >= btc_ema
        entry_ready=[]
        ...
        for live in entry_ready:
            ...
            if sector_position_count(live.symbol, tuple(loop.account.positions.keys())) >= RISK_CONFIG.max_positions_per_sector:
                diagnostics["sector_rejections"]+=1; continue
            if exceeds_correlation_limit(live.symbol, tuple(loop.account.positions.keys()), history,
                                         threshold=RISK_CONFIG.max_pairwise_correlation,window=RISK_CONFIG.correlation_window):
                diagnostics["correlation_rejections"]+=1; continue
