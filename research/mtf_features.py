...
def _base(df):
    x=df.copy(); x["timestamp"]=pd.to_datetime(x["timestamp"],utc=True); x=x.sort_values("timestamp").drop_duplicates("timestamp")
...
def _htf_available(frame,suffix):
    selected=frame[["time","close","ema20","ema50","ema200","rsi","macd_hist","adx","atr_pct","vol_ratio","vol_regime"]].copy(); selected["available"]=selected["time"].shift(-1); selected=selected.dropna(subset=["available"])
...
def add_btc_context(asset_features, btc_features):
    ...
    left=asset_features.copy(); right=btc_features[["timestamp","ema20_1h","ema50_1h","ema200_1h","adx1h","vol_regime_1h"]].copy()
    right=right.sort_values("timestamp").drop_duplicates("timestamp")
    right["available"]=(right["timestamp"].dt.floor("1h") + pd.Timedelta(hours=1))
