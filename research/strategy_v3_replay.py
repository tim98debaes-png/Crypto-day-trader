...
from strategy_v3 import V3Config, _atr, score_setup, select_setup, should_exit
...
def _candidate(symbol, timestamp, bars, account, histories, btc_bars, config, diagnostics):
    ...
    if atr <= 0:
        ...
        return None
    risk_pct = __import__("strategy_v3").position_risk_pct(setup, config)
    ...
    admission = admit(symbol, risk_pct, snapshot, histories_dict, max_positions=config.max_open_positions,
    ...
    return {"symbol": symbol, "direction": setup.direction, "score": setup.score, "atr": atr,
            "risk_pct": risk_pct, "regime": setup.regime}
...
def run_v3(frames: dict[str, pd.DataFrame], config: ReplayConfig = ReplayConfig(), v3: V3Config = V3Config()) -> tuple[dict, dict]:
    ...
    account = PaperAccount(capital=config.capital, cash=config.capital, risk_pct=config.risk_pct, fee_pct=config.fee_pct,
                           slippage_pct=config.slippage_pct, max_daily_loss_pct=config.max_daily_loss_pct)
    histories = {symbol: deque(maxlen=60) for symbol in frames}
    atr_by_symbol = defaultdict(float)
    cursor = {symbol: 0 for symbol in frames}
    pending = {}
    stats = _stats()
    diagnostics = {"strategy": "V3", "signals": 0, "opened_trades": 0, "below_threshold": 0,
                   "insufficient_mtf": 0, "invalid_atr": 0, "portfolio_rejections": defaultdict(int),
                   "partials": 0, "partial_pnl": 0.0, "max_open_positions": 0,
    ...
    for timestamp in timestamps:
        ...
        for symbol, candidate in list(pending.items()):
            ...
            if row is None or symbol in account.positions:
                continue
            try:
                account.open_position(symbol=symbol, direction=candidate["direction"], price=float(row["open"]),
                                      stop_distance=min(candidate["atr"] * v3.stop_atr_multiple, float(row["open"]) * v3.max_stop_pct),
                                      rr=2.0, timestamp=str(timestamp), strategy_score=int(candidate["score"] * 100),
                                      strategy_tier=f"V3_{candidate['regime']}", risk_pct_override=candidate["risk_pct"])
...
def main() -> None:
    ...
    report = {"schema_version": 1, "status": "EXECUTION_COMPLETE", "procedure": "strategy_v3_event_replay",
              "data_interval": "1m", "strategy": "V3", "execution": {"capital": 1000.0, "risk_pct": 0.5, "fee_pct": 0.1,
              "slippage_pct": 0.02, "max_daily_loss_pct": 3.0}, "result": result, "diagnostics": diagnostics}
