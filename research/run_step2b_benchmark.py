"""Run reproducible Step 2b A/B/C portfolio benchmark."""
...
def precompute_signals(features,params):
    long_s,short_s=legacy_signals(features,params); out={}; prices=features["close"].astype(float).tolist()
...
def main():
    ...
    for strategy in ("A","B","C"):
        result=run_strategy(features,maps,strategy,rows); results[strategy]={"label":{"A":"LEGACY","B":"CURRENT","C":"HYBRID"}[strategy],**result.summary()}
    out=Path(a.output); out.mkdir(parents=True,exist_ok=True); report={"schema_version":2,"status":"EXECUTION_COMPLETE","start":a.start,"end":a.end,"symbols":len(features),"candles":sum(len(x) for x in features.values()),"fixed_legacy_candidate":params,"execution":{"capital":1000.0,"risk_pct":0.5,"fee_pct":0.1,"slippage_pct":0.02,"max_daily_loss_pct":3.0},"results":results}
