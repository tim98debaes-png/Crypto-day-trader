...
def run_benchmark(frames: dict[str, pd.DataFrame], config: ReplayConfig, v3: V3Config, folds: int = 3) -> dict:
    # Two full replays establish the apples-to-apples V2/V3 comparison.
    # OOS uses three chronological slices: enough for the promotion gate while
    # avoiding the fourth redundant full replay that previously hit the 180-min CI limit.
    v2_result, v2_diag = run_v2(frames, config)
    v3_result, v3_diag = run_v3(frames, config, v3)
    v2_metrics = _metric_block({"result": v2_result, "diagnostics": v2_diag})
    ...
    for i in range(folds):
        ...
        oos.append({
            ...
            "end": str(fold_end),
            "metrics": metrics,
            "regime_trade_counts": diag.get("regime_trade_counts", {}),
            "exits": diag.get("exits", {}),
        })
