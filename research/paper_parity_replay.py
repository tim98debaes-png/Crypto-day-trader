...
def _manage_position(account: PaperAccount, row: pd.Series, history: deque[float], stats: dict, diag: dict) -> None:
    ...
    if position.partial_taken:
        atr = max(close * max(_volatility(history), 0.05) / 100.0 * 0.5, close * 0.001)
        account.update_trailing_stop(symbol, close, atr)
    signal_exit = exit_signal(list(history), position.direction)
    time_stop = account.position_age_minutes(symbol, timestamp) >= RISK_CONFIG.time_stop_minutes
    trailing_stop_hit = low <= position.stop_price if position.direction == "LONG" else high >= position.stop_price
    if trailing_stop_hit or time_stop or signal_exit:
        if trailing_stop_hit:
            reason, trigger = "SL", position.stop_price
        elif time_stop:
            reason, trigger = "TIME_STOP", close
        else:
            reason, trigger = "SIGNAL", close
        _record(stats, account.close_position(trigger, reason, timestamp, symbol=symbol, trigger_price=trigger)); diag["exits"][reason] += 1


def _options(row: pd.Series, history: deque[float], account: PaperAccount, mode: str, diag: dict) -> list[dict]:
    symbol = str(row["symbol"])
    if symbol in account.positions or len(history) < 12:
        return []
    vol = _volatility(history)
    if not RISK_CONFIG.volatility_floor_pct <= vol <= RISK_CONFIG.volatility_ceiling_pct:
        diag["volatility_rejections"] += 1
        return []
    options = []
    for direction in ("LONG", "SHORT"):
        ready, reason, score, _ = entry_signal_details(list(history), direction)
        if not ready:
            diag["entry_rejections"][reason] = diag["entry_rejections"].get(reason, 0) + 1
            continue
...
def run_replay(frames: dict[str, pd.DataFrame], mode: str, config: ReplayConfig = ReplayConfig()) -> tuple[dict, dict]:
    ...
    history = {symbol: deque(maxlen=config.history_size) for symbol in data}
    cursor = {symbol: 0 for symbol in data}
    pending: dict[str, dict] = {}
    stats = _stats()
    diag = {"mode": mode, "entry_rejections": {}, "regime_rejections": 0, "btc_rejections": 0, "volatility_rejections": 0, "ambiguous_direction_rejections": 0, "partials": 0, "partial_pnl": 0.0, "exits": {"SL": 0, "TP": 0, "SIGNAL": 0, "TIME_STOP": 0}, "opened_trades": 0, "max_open_positions": 0}
    curve: list[float] = []
    timestamps = sorted(set().union(*(set(frame["timestamp"]) for frame in data.values())))
    for timestamp in timestamps:
        ...
        for symbol, row in rows.items():
            ...
            if len(h) < config.min_history:
                continue
            change = (h[-1] / h[0] - 1.0) * 100.0
            snapshots.append(AssetSnapshot(symbol, float(row["close"]), float(row["quote_volume_24h"]), change, _volatility(h)))
        ...
        for ranked_item in ranked:
            ...
            for option in _options(row, history[ranked_item.symbol], account, mode, diag):
                option["candidate_score"] = ranked_item.score
                option["stop_distance"] = _stop_distance(float(row["close"]), option["volatility_pct"])
        ...
        diag["max_open_positions"] = max(diag["max_open_positions"], len(account.positions))
...
def main() -> None:
    ...
    (output / "paper_parity_report.json").write_text(json.dumps(report, indent=2, default=str) + "\n", encoding="utf-8")
