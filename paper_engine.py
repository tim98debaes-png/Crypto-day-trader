...
class PaperAccount:
    ...
    def open_position(self, symbol: str, direction: str, price: float, stop_distance: float, rr: float,
                      timestamp: Optional[str] = None, *, strategy_ready: bool = True,
                      heartbeat_age_seconds: float | None = 0.0, paper_mode: bool = True) -> PaperPosition:
        timestamp = timestamp or datetime.now(timezone.utc).isoformat(); self._roll_day(timestamp)
        symbol = str(symbol); direction = direction.upper()
        ...
        guard = evaluate_entry_guard(paper_mode=paper_mode, strategy_ready=strategy_ready,
                                     heartbeat_age_seconds=heartbeat_age_seconds,
                                     drawdown_pct=self.daily_loss_pct(price, symbol), max_drawdown_pct=20.0)
        ...
        effective_risk_pct = min(self.risk_pct, self.risk_config.max_risk_pct_per_trade) * self._risk_multiplier(timestamp)
        risk_amount = self.cash * effective_risk_pct / 100.0
        ...
        target = entry + stop_distance * rr if direction == "LONG" else entry - stop_distance * rr
        ...
        position = PaperPosition(symbol, direction, entry, quantity, stop, target, timestamp, entry_fee,
                                  quantity, stop_distance, risk_amount, False, price, price)
        self.positions[symbol] = position
        self.audit_log.append({"event":"OPEN","symbol":symbol,"direction":direction,"price":entry,"quantity":quantity,
                               "entry_fee":entry_fee,"risk_amount":risk_amount,"risk_pct":effective_risk_pct,"timestamp":timestamp})
        return position
    ...
    def update_trailing_stop(self, symbol: str, price: float, atr_distance: float) -> float:
        ...
        if position.direction == "LONG":
            ...
            candidate = position.highest_price - self.risk_config.trailing_atr_multiple * atr_distance
            ...
            candidate = position.lowest_price + self.risk_config.trailing_atr_multiple * atr_distance
            if candidate < position.stop_price and candidate > price: position.stop_price = candidate
    ...
    def take_partial_profit(self, symbol: str, price: float, timestamp: Optional[str] = None) -> float:
        ...
        exit_fee = exit_price * quantity * self.fee_pct / 100.0
        allocated_entry_fee = position.entry_fee * (quantity / max(position.initial_quantity, quantity))
        pnl = gross - exit_fee - allocated_entry_fee; self.cash += gross - exit_fee; position.quantity -= quantity; position.partial_taken = True
        ...
        self.audit_log.append({"event":"PARTIAL_CLOSE","symbol":symbol,"direction":position.direction,"price":exit_price,
                               "quantity":quantity,"pnl":pnl,"reason":"PARTIAL_TP","timestamp":timestamp})
        return float(pnl)
    ...
    def close_position(self, price: float, reason: str = "SIGNAL", timestamp: Optional[str] = None, symbol: Optional[str] = None) -> float:
        ...
        exit_fee = exit_price * position.quantity * self.fee_pct / 100.0; pnl = gross - exit_fee
        self.cash += gross - exit_fee; del self.positions[symbol]
        ...
        self.audit_log.append({"event":"CLOSE","symbol":symbol,"direction":position.direction,"price":exit_price,"quantity":position.quantity,
                               "gross_pnl":gross,"exit_fee":exit_fee,"pnl":pnl,"reason":reason,"timestamp":timestamp})
        return float(pnl)
    ...
    def snapshot(self, mark_price: Optional[float] = None, symbol: Optional[str] = None) -> dict:
        return {"cash":round(self.cash,8),"equity":round(self.equity(mark_price,symbol),8),"daily_loss_pct":round(self.daily_loss_pct(),6),
                "position_count":len(self.positions),"open_risk_pct":round(self.open_risk_pct(),6),"loss_streak":self.loss_streak,
                "cooldown_until":self.cooldown_until,"positions":[{"symbol":p.symbol,"direction":p.direction,"entry_price":p.entry_price,
                "quantity":p.quantity,"stop_price":p.stop_price,"target_price":p.target_price,"partial_taken":p.partial_taken} for p in self.positions.values()],
