...
class PaperAccount:
    ...
    def _roll_day(self, timestamp: Optional[str] = None) -> None:
        ...
        if day != self.current_day:
            self.current_day=day; self.day_start_equity=self.equity(); self.loss_streak=0; self.cooldown_until=None; self.symbol_cooldown_until={}
    def has_position(self,symbol:str)->bool: return str(symbol).upper() in self.positions
    def equity(self,mark_price:Optional[float]=None,symbol:Optional[str]=None)->float:
        if mark_price is not None and symbol is not None: self.last_prices[str(symbol).upper()]=float(mark_price)
        unrealized=0.0
    ...
    def _symbol_cooldown_active(self,symbol:str,timestamp:Optional[str])->bool:
        until=self.symbol_cooldown_until.get(str(symbol).upper())
        if not until: return False
        return self._parse_timestamp(timestamp)<self._parse_timestamp(until)
    ...
    def can_open(self,mark_price:float,symbol:str,timestamp:Optional[str]=None,*,risk_pct_override:float|None=None)->bool:
        ...
        return (not self.has_position(symbol) and not self._symbol_cooldown_active(symbol,timestamp)
                and len(self.positions)<self.risk_config.max_open_positions
    ...
    def open_position(self,symbol:str,direction:str,price:float,stop_distance:float,rr:float,timestamp:Optional[str]=None,*,strategy_ready:bool=True,heartbeat_age_seconds:float|None=0.0,paper_mode:bool=True,risk_pct_override:float|None=None,strategy_score:int|None=None,strategy_tier:str|None=None)->PaperPosition:
        timestamp=timestamp or datetime.now(timezone.utc).isoformat(); self._roll_day(timestamp); symbol=str(symbol).upper(); direction=direction.upper()
    ...
    def update_trailing_stop(self,symbol:str,price:float,atr_distance:float)->float:
        position=self.positions.get(str(symbol).upper())
    ...
    def take_partial_profit(self,symbol:str,price:float,timestamp:Optional[str]=None)->float:
        ...
        return float(pnl)
    def close_position(self,price:float,reason:str="SIGNAL",timestamp:Optional[str]=None,symbol:Optional[str]=None,*,trigger_price:Optional[float]=None)->float:
        if symbol is None:
            position=self.position
            if position is None: raise RuntimeError("no open paper position")
            symbol=position.symbol
        else:
            symbol=str(symbol).upper(); position=self.positions.get(symbol)
        ...
        cooldown_minutes=self.risk_config.loss_reentry_cooldown_minutes if pnl<0 else self.risk_config.win_reentry_cooldown_minutes
        self.symbol_cooldown_until[symbol]=(self._parse_timestamp(timestamp)+timedelta(minutes=cooldown_minutes)).isoformat()
        if self.loss_streak>=8: self.cooldown_until=(self._parse_timestamp(timestamp)+timedelta(minutes=30)).isoformat()
        trigger=price if trigger_price is None else float(trigger_price)
        execution_gap_pct=abs(float(price)-trigger)/max(abs(trigger),1e-12)*100.0
        ...
        self.audit_log.append({"event":"CLOSE","symbol":symbol,"direction":position.direction,"price":exit_price,"quantity":position.quantity,"gross_pnl":gross,"entry_fee":position.entry_fee,"exit_fee":exit_fee,"pnl":pnl,"reason":reason,"timestamp":timestamp,"intended_risk_amount":position.risk_amount,"intended_stop_price":position.initial_stop_price,"initial_stop_price":position.initial_stop_price,"current_stop_price":position.stop_price,"initial_stop_gap_pct":initial_stop_gap_pct,"execution_gap_pct":execution_gap_pct,"actual_loss_amount":actual_loss,"risk_to_actual_ratio":risk_to_actual_ratio,"reentry_cooldown_minutes":cooldown_minutes})
        return float(pnl)
    ...
    def snapshot(self,mark_price:Optional[float]=None,symbol:Optional[str]=None)->dict:
        return {"cash":round(self.cash,8),"equity":round(self.equity(mark_price,symbol),8),"daily_loss_pct":round(self.daily_loss_pct(),6),"position_count":len(self.positions),"open_risk_pct":round(self.open_risk_pct(),6),"loss_streak":self.loss_streak,"cooldown_until":self.cooldown_until,"symbol_cooldowns":dict(self.symbol_cooldown_until),"positions":[{"symbol":p.symbol,"direction":p.direction,"entry_price":p.entry_price,"quantity":p.quantity,"stop_price":p.stop_price,"target_price":p.target_price,"partial_taken":p.partial_taken} for p in self.positions.values()],"audit_events":len(self.audit_log)}
