...
class BacktestResult:
    ...
    @property
    def pnl(self) -> float: return self.final_equity - self.initial_capital
    @property
    def return_pct(self) -> float: return (self.final_equity / self.initial_capital - 1.0) * 100.0 if self.initial_capital > 0 else 0.0
    @property
    def max_drawdown_pct(self) -> float:
        peak=self.initial_capital; maximum=0.0
        for row in self.equity_curve:
            equity=float(row["equity"]); peak=max(peak,equity)
            if peak: maximum=max(maximum,(peak-equity)/peak*100.0)
        return maximum
    def summary(self) -> dict:
        closes=[x for x in self.trades if x.get("event")=="CLOSE"]
        wins=[x for x in closes if float(x.get("pnl",0))>0]; losses=[x for x in closes if float(x.get("pnl",0))<0]
        gross_profit=sum(float(x.get("pnl",0)) for x in wins); gross_loss=abs(sum(float(x.get("pnl",0)) for x in losses))
        return {"initial_capital":self.initial_capital,"final_equity":self.final_equity,"pnl":self.pnl,"return_pct":round(self.return_pct,10),"max_drawdown_pct":round(self.max_drawdown_pct,10),"closed_trades":len(closes),"wins":len(wins),"losses":len(losses),"win_rate_pct":round(len(wins)/len(closes)*100,10) if closes else 0.0,"profit_factor":gross_profit/gross_loss if gross_loss else (float("inf") if gross_profit else 0.0)}
...
class HistoricalBacktester:
    ...
    def run(self,candles,signal_provider):
        rows=[dict(candle) for candle in candles]; account=PaperAccount(**self.config,cash=self.config["capital"]); equity_curve=[]; previous_timestamp=None
        for row in rows:
            timestamp=str(row.get("timestamp") or row.get("time") or "")
            if previous_timestamp is not None and timestamp and timestamp<previous_timestamp: raise ValueError("candles must be ordered chronologically")
            previous_timestamp=timestamp or previous_timestamp; high,low,close=self._ohlc(row)
            if account.position is not None:
                exit_event=self._exit_trigger(account.position,high,low)
                if exit_event is not None:
                    reason,trigger=exit_event; account.close_position(trigger,reason,timestamp or None,trigger_price=trigger)
            signal=signal_provider(row) or {}; action=str(signal.get("action","WAIT")).upper(); symbol=str(row.get("symbol","BACKTEST"))
            if account.position is None and action in {"LONG","SHORT"}:
                try:
                    account.open_position(symbol=symbol,direction=action,price=close,stop_distance=float(signal["stop_distance"]),rr=float(signal.get("rr",2.0)),timestamp=timestamp or None)
            ...
            equity_curve.append({"timestamp":timestamp,"equity":round(account.equity(close,symbol),8)})
        if account.position is not None and rows:
            last=rows[-1]; final_close=float(last["close"]); account.close_position(final_close,"END",str(last.get("timestamp") or last.get("time") or "") or None,trigger_price=final_close)
            if equity_curve: equity_curve[-1]["equity"]=round(account.equity(),8)
