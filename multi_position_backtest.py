...
@dataclass
class MultiPositionBacktestResult:
    initial_capital: float
    final_equity: float
    trades: list[dict] = field(default_factory=list)
    equity_curve: list[dict] = field(default_factory=list)
    @property
    def pnl(self): return self.final_equity - self.initial_capital
    @property
    def return_pct(self): return (self.final_equity / self.initial_capital - 1) * 100 if self.initial_capital > 0 else 0
    @property
    def max_drawdown_pct(self):
        ...
        for row in self.equity_curve:
            ...
            if peak>0: maximum=max(maximum,(peak-equity)/peak*100)
        return maximum
    def summary(self):
        closes=[e for e in self.trades if e.get('event')=='CLOSE']
        wins=[e for e in closes if float(e.get('pnl',0))>0]; losses=[e for e in closes if float(e.get('pnl',0))<0]
        gp=sum(float(e.get('pnl',0)) for e in wins); gl=abs(sum(float(e.get('pnl',0)) for e in losses))
    ...
    def run(self,candles,signal_provider):
        ...
        if rows:
            ...
            for symbol in list(account.positions):
                final_price=account.last_prices.get(symbol)
                if final_price is not None: account.close_position(final_price,'END',timestamp,symbol=symbol,trigger_price=final_price)
            if equity_curve: equity_curve[-1]['equity']=round(account.equity(),8); equity_curve[-1]['position_count']=0
        return MultiPositionBacktestResult(self.config['capital'],round(account.equity(),8),list(account.audit_log),equity_curve)
