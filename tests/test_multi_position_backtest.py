...
def test_multiple_symbols_can_hold_positions_simultaneously():
    rows = [
        ...
        {'timestamp':'2026-01-01T00:01:00+00:00','symbol':'BBB','open':200,'high':201,'low':199,'close':200},
    ]
    def signal(row):
        return {'action':'LONG','stop_distance':1,'rr':2} if row['timestamp'].endswith('00:00:00+00:00') else {'action':'WAIT'}
...
def test_signal_on_final_candle_is_not_executed_without_next_open():
    rows = [{'timestamp':'2026-01-01T00:00:00+00:00','symbol':'AAA','open':100,'high':100,'low':100,'close':100}]
...
def test_signal_close_executes_on_next_candle_open():
    ...
    def signal(row):
        if row['timestamp'].endswith('00:00:00+00:00'): return {'action':'LONG','stop_distance':5,'rr':10}
        if row['timestamp'].endswith('00:02:00+00:00'): return {'action':'CLOSE'}
        return {'action':'WAIT'}
    result = MultiPositionBacktester(risk_pct=0.5, slippage_pct=0).run(rows, signal)
...
def test_intrabar_stop_closes_even_when_candle_closes_above_stop():
    rows = [
        {'timestamp':'2026-01-01T00:00:00+00:00','symbol':'AAA','open':100,'high':100,'low':100,'close':100},
        {'timestamp':'2026-01-01T00:01:00+00:00','symbol':'AAA','open':100,'high':101,'low':98,'close':100},
    ]
    result = MultiPositionBacktester(risk_pct=0.5, slippage_pct=0).run(rows, lambda r: {'action':'LONG','stop_distance':1,'rr':2} if r['timestamp'].endswith('00:00:00+00:00') else {'action':'WAIT'})
    closes = [e for e in result.trades if e.get('event') == 'CLOSE']
...
def test_both_stop_and_target_is_conservative_stop():
    rows = [
        {'timestamp':'2026-01-01T00:00:00+00:00','symbol':'AAA','open':100,'high':100,'low':100,'close':100},
        {'timestamp':'2026-01-01T00:01:00+00:00','symbol':'AAA','open':100,'high':102,'low':98,'close':100},
    ]
    result = MultiPositionBacktester(risk_pct=0.5, slippage_pct=0).run(rows, lambda r: {'action':'LONG','stop_distance':1,'rr':1} if r['timestamp'].endswith('00:00:00+00:00') else {'action':'WAIT'})
    closes = [e for e in result.trades if e.get('event') == 'CLOSE']
    assert closes and closes[0]['reason'] == 'SL'
